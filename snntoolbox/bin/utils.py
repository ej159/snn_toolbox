...
def update_setup(config_filepath):
    ...
    if simulator == 'brian2' or 'spiNNaker' and not config.getboolean('tools', 'convert'):
        print(dedent("""\ \n
            SNN toolbox Warning: When using Brian 2 or SpiNNaker simulators, you need to
...
def initialize_simulator(config):
    ...
    if simulator in config_string_to_set_of_strings(
        ...
        sim.setup(timestep=config.getfloat('simulation', 'dt'), time_scale_factor=config.getfloat('simulation', 'time_scale_factor'))
        return sim
    if simulator == 'brian2':
